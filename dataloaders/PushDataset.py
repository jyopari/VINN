...
class PushDataset(Dataset):
            ...
            for frame in action_dict:
                ...
                if(self.params['representation'] == 1):
                    ...
                    if(self.params['bc_model'] == 'BC_Full'):
                        self.actions.append(torch.FloatTensor(action_dict[frame][0]))
    ...
    def __getitem__(self, index):
        if(self.params['representation'] == 1):
            if(self.params['bc_model'] == 'BC_Full'):
                return((self.img_tensors[index], self.actions[index], self.paths[index]))
            else:
                return(self.img_tensors[index])
        else:
            return((self.representations[index], self.actions[index], self.paths[index]))
