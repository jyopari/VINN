...
class getOutput (object):
    ...
    def extract_data_temporal(self, t):
        ...
        for run_index in tqdm(range(len(runs))):
            ...
            for frame_index in range(len(key_list)):
                try:
                    img = Image.open(run+'/images_linear/'+key_list[frame_index])
                ...
                temporal_paths.append(runs[run_index]+'/'+key_list[frame_index])
    ...
    def show_subscribed_image(self):
        ...
        while True:
            ...
            translation_tensor = self.translation_model(embedding).tolist() 
            rotation_tensor = self.rotation_model(embedding).tolist() 
            gripper_tensor = [torch.argmax(self.gripper_model(embedding))]
